...
export interface EventManager extends PeBLPlugin {
  ...
  saveEvents(identity: string, stmts: XApiStatement[]): void; // Store the events for this user
  deleteEvent(identity: string, id: string): void; //Removes the event with the specified id
}

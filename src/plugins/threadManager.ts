...
export class DefaultThreadManager extends PeBLPlugin implements ThreadManager {
  ...
  constructor(sessionData: SessionDataManager) {
    ...
    this.addMessageTemplate(new MessageTemplate("storeMessage",
      this.validateThreadWritePermission,
      (payload: { [key: string]: any }) => {
        this.storeMessage(payload.message);
      }));
    ...
    this.addMessageTemplate(new MessageTemplate("subscribeThread",
      this.validateThreadReadPermission,
      (payload: { [key: string]: any }) => {
        this.subscribeThread(payload.identity, payload.thread, payload.groupId);
      }));

    this.addMessageTemplate(new MessageTemplate("unsubscribeThread",
      this.validateThreadReadPermission,
      (payload: { [key: string]: any }) => {
        this.unsubscribeThread(payload.identity, payload.thread, payload.groupId);
      }));

    this.addMessageTemplate(new MessageTemplate("deleteMessage",
      this.validateMessageOwnership,
      (payload: { [key: string]: any }) => {
        this.deleteMessage(payload.thread, payload.messageId, payload.groupId);
      }));
  }
  ...
  subscribeThread(userId: string, thread: string, groupId?: string): void {
    if (groupId)
      thread = this.getGroupScopedThread(thread, groupId);
    this.sessionData.setHashValues('users:thread', [thread, userId]);
  }

  unsubscribeThread(userId: string, thread: string, groupId?: string): void {
    if (groupId)
      thread = this.getGroupScopedThread(thread, groupId);
    this.sessionData.deleteHashValue('users:thread', thread, (deleted) => { });
  }
  ...
  deleteMessage(thread: string, messageId: string, groupId?: string): void {
    if (groupId)
      thread = this.getGroupScopedThread(thread, groupId);
    this.sessionData.deleteHashValue('threads:' + thread, messageId, (deleted) => { });
  }
}